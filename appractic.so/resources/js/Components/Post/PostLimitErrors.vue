...
<template>
    <div v-if="show"
         class="w-full flex items-center row-px py-xs md:py-md flex-row border-b border-primary-200 text-red-500 bg-red-50">
        ...
        <div>
            <div v-if="accountsHitTextLimit">
                <p v-for="item in accountsHitTextLimit">
                    <span class="capitalize text-primary-900">{{ resolveProvider(item.provider) }}</span> can only fit {{ item.limit }}
                    characters. <span v-if="item.account_name">Check out the <span class="font-semibold text-primary-900">{{
                        item.account_name
                    }}</span> version.</span>
                </p>
            </div>
            ...
            <div v-if="accountsHitMediaLimit">
                <p v-for="item in accountsHitMediaLimit">
                    <span v-if="item.mixing.hit">
                        <span>You cannot mix video, gif and images on <span class="text-primary-900">{{ resolveProvider(item.mixing.provider) }}</span>.</span>
                    </span>
                    <span v-else>
                    <span v-if="item.photos.hit">
                        <span class="capitalize text-primary-900">{{
                    ...
                    <span v-if="item.account_name"> Check out the <span class="font-semibold text-primary-900">{{
                            item.account_name
                        }}</span> version.</span>
                </p>
            </div>
