...
<template>
    <div class="bg-white border border-primary-200 rounded-lg p-sm w-fit">
        <div class="flex flex-wrap items-center space-x-1">
            <template v-for="(link, index) in formattedLinks">
                ...
                <Link v-else :key="`link-${index}`" disabled
                      ...
                      :class="[linkClass, { 'bg-primary-500 text-white': link.active, 'text-primary-600 hover:text-primary-700 hover:bg-primary-50 focus:text-primary-700 focus:bg-primary-50': !link.active, '!px-xs': link.component }]"
                      :href="link.url">
                    <template v-if="link.component">
                        <component :is="link.component"/>
                    </template>
